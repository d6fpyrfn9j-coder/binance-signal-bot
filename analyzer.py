...
def _day_trade_risk_pct(symbol: str) -> float:
    # Wider stops so normal crypto noise / stop-hunts don't shake us out before the
    # move. Risk per trade stays fixed (position size auto-shrinks); target scales
    # with this too, so R/R is preserved.
    if symbol == "BTCUSDT":
        return 0.015
